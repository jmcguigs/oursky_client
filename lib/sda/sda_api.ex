...
def create_target(tle_line_1, tle_line_2, name, opts \\ []) do
  ...
  """
  def schedule_volumetric_search(target_uuid, radial_3_sigma \\ 20000.0,
        intrack_3_sigma \\ 50000.0, crosstrack_3_sigma \\ 10000.0, covariance_epoch \\ nil) do
    response = "https://api.prod.oursky.ai/v2/search/ric-volumetric"
    |> Req.post!(
      json: %{
        targetId: target_uuid,
        radial3SigmaMeters: radial_3_sigma,
        intrack3SigmaMeters: intrack_3_sigma,
        crosstrack3SigmaMeters: crosstrack_3_sigma,
        ricStdevEpoch: covariance_epoch
      },
      auth: {:bearer, Application.get_env(:oursky_client, :access_token)}
    )
  ...
  """
  def cancel_volumetric_search(search_uuid) do
    response = "https://api.prod.oursky.ai/v2/search/ric-volumetric?id=#{search_uuid}"
    |> Req.delete!(
      auth: {:bearer, Application.get_env(:oursky_client, :access_token)}
    )
